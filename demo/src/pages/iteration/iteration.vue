  ...
  <div class="container-transfer" :style="{minHeight: '100vh'}">
    <div class="transBody">
      <div class="transBodyL">
        <h3>迭代名称-{{titleName}}<span class="totalNum"><i>{{nowlNum}}/</i>{{totalNum}}</span></h3>
        <Checkbox
            :value="checkAll"
          ...
          <ul>
            <li v-for="item in obj" :key="item.type">
              <div class="tranHeader">
                <Checkbox :label="item.type">{{item.name}}</Checkbox>
      ...
      <div class="transBodyC">
        <Button type="info" long icon="chevron-left" @click="toLeft">To left</Button>
        <Button long icon="chevron-right" @click="toRight">To right</Button>
      </div>
      <div class="transBodyR">
        <div class="trans-top">
          ...
          <Select v-model="model1" style="width:100px">
           ...
           <Input v-model="search"  placeholder="search" style="width:180px"
           icon="android-search">
           </Input>
        </div>
...
export default {
  ...
  methods: {
    ...
    toLeft() {
      if (this.checkAllGroupR.length < 1) {
        ...
        this.checkAllGroupR.forEach(element => {
          ...
          this.obj.push(this.obj2.find(n => n.type == element));
          this.obj2.splice(this.obj2.findIndex(n => n.type == element), 1);
        });
        this.checkAll = false;
    ...
    toRight() {
      if (this.checkAllGroup.length < 1) {
        ...
        this.checkAllGroup.forEach(element => {
          // alert(this.obj2.find(n => n.type == element))
          this.obj2.push(this.obj.find(n => n.type == element));
          this.obj.splice(this.obj.findIndex(n => n.type == element), 1);
        });
        this.checkAll = false;
        this.checkAllR = false;
      }
    ...
    checkAllGroupChange(data) {
      if (data.length < this.obj.length) {
        this.checkAll = false;
      } else {
        this.checkAll = true;
      }
    ...
    checkAllGroupChangeR(data) {
      if (data.length < this.obj2.length) {
        this.checkAllR = false;
      } else {
        this.checkAllR = true;
      }
